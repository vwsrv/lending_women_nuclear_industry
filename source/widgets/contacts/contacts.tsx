...
export const Contacts: React.FC<contactsProps> = ({ contactInfo, documents }) => {
  return (
    <section className="bg-indigo p-20 text-gray max-w-[1440px]">
      ...
      <div className="leading-10 text-3xl/10 grid grid-cols-2 gap-9 mb-28">
        <ul>
          {
            contactInfo.map(({bgname, link, text}, index) => { // {name, link, text}
              ...
              return (
              <li key={index} className={"flex flex-row py-5 pl-16 " + bgname + " bg-no-repeat bg-[left_center]"}>
                <a href={link} target="_blank" rel="nofollow noindex noopener" className="underline">{text}</a>
              </li>
              )
            })
          }
        ...
        <div className="bg-logo-dark bg-no-repeat bg-top-center">
          <form className="bg-magenta text-gray mt-64">Форма &quot;подписаться&quot; и кнопка &quot;поддержать проект&quot;</form>
        </div>
      </div>
      <h3 className="uppercase text-3xl/10 font-bold mb-9 font-normal">Документы</h3>
      <ul className="uppercase leading-10 text-2xl font-bold grid grid-cols-2 gap-x-9 gap-y-10">
        {
          documents.map(({name, link}, index) => {
            ...
            <li key={index} className="flex flex-row">
              <a href={link} target="_blank" rel="nofollow noindex noopener" className="underline">{name}</a>
            </li>
            )
          })
        }
